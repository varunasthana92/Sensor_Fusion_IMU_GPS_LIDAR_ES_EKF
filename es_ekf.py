...
def measurement_update(sensor_var, p_cov_check, y_k, p_check, v_check, q_check):
    ...
    delta_e = Kk @ (y_k - p_check).reshape(3,1)
    delta_e = delta_e.ravel()
    # 3.3 Correct predicted state
    p_hat = p_check + delta_e[0:3]
    v_hat = v_check + delta_e[3:6]
    q_temp = Quaternion(euler = delta_e[6:])
    q_hat = q_temp.quat_mult_left(q_check)
...
for k in range(1, imu_f.data.shape[0]):  # start at 1 b/c we have initial prediction from gt
    ...
    rot_mat = Quaternion(*q_est[k-1]).to_mat()
    p_check = p_est[k-1] + delta_t*v_est[k-1] + 0.5*delta_t**2 * (rot_mat @ imu_f.data[k-1] + g)
    v_check = v_est[k-1] + delta_t*(rot_mat @ imu_f.data[k-1] + g)
    ...
    F[3:6, 6:9] = -skew_symmetric(rot_mat @ imu_f.data[k-1].reshape(3,1))*delta_t
    Q = np.eye(6)
    Q[:3,:] *= var_imu_f*delta_t*delta_t
    Q[3:,:] *= var_imu_w*delta_t*delta_t
    # 2. Propagate uncertainty
    p_cov_check = F @ p_cov[k-1] @ F.T + l_jac @ Q @ l_jac.T
    # 3. Check availability of GNSS and LIDAR measurements
    if gnss_i < gnss.t.shape[0] and imu_f.t[k-1] == gnss.t[gnss_i]:
    	p_check, v_check, q_check, p_cov_check = measurement_update(R_gnss, p_cov_check, gnss.data[gnss_i].T, p_check, v_check, q_check)
    	gnss_i +=1

    if lidar_i < lidar.t.shape[0] and imu_f.t[k-1] == lidar.t[lidar_i]:
    	p_check, v_check, q_check, p_cov_check = measurement_update(R_lidar, p_cov_check, lidar.data[lidar_i].T, p_check, v_check, q_check)
    	lidar_i +=1
    # Update states (save)
    p_est[k] = p_check
    v_est[k] = v_check
